...
const analyzePlank = async (landmarks) => {
  ...
  try {
    const response = await axios.post(
      "http://localhost:3000/exercises/plank",
      { points },
      { timeout: 5000 }
    );
    ...
    if (
      response.data &&
      (response.data.back_too_low !== undefined ||
        response.data.back_too_high !== undefined)
    ) {
      return {
...
const analyzeBicepCurls = async (landmarks) => {
  ...
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/exercises/bicep-curls",
      { points },
      { timeout: 10000 }
    );
    ...
    if (
      response.data &&
      (response.data.leaned_back !== undefined || response.data.correct !== undefined)
    ) {
...
const analyzeLunges = async (landmarks) => {
  ...
  try {
    const response = await axios.post(
      "http://localhost:3000/exercises/lunges",
      { points },
      { timeout: 5000 }
    );
    ...
    if (
      response.data &&
      (response.data.knee_over_toe !== undefined || response.data.correct !== undefined)
    ) {
      return {
    ...
    const setupCamera = async () => {
      ...
      try {
        ...
        camera = new Camera(videoRef.current, {
          onFrame: () => {
            const now = Date.now();
            if (now - lastFrameTime >= 200) { // 100ms interval for 10 fps
              lastFrameTime = now;
