...
def get_coordinates(landmarks, part):
...
def lateralRaises(frame):
    ...
    if not results.pose_landmarks:
        return json.dumps({"error": "Landmarks not detected"})
...
def inclinedDumbbellPress(frame):
    rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    results = pose.process(rgb_frame)
...
def ropeOverheadExtensions(frame):
    rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    results = pose.process(rgb_frame)
